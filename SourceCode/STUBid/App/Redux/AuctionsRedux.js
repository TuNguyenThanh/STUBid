import { createReducer, createActions } from 'reduxsauce'
import Immutable from 'seamless-immutable'

/* ------------- Types and Action Creators ------------- */

const { Types, Creators } = createActions({
  auctionsRequest: ['category', 'page'],
  auctionsSuccess: ['data'],
  auctionsFailure: ['error'],

  bidProductRequest: ['token', 'auctionId', 'accountId', 'priceBid', 'buyNow'],
  bidProductSuccess: ['bidSuccess'],
  bidProductFailure: ['error'],

  setData: ['listData'],

  myAuctions: ['myListAuction'],
  myAuctionsClose: ['myListAuctionClose'],
  myAuctionsHanding: ['myAuctionsHanding'],
  myAuctionsUnActive: ['myAuctionsUnActive'],
  myCloseAuctionsActive: ['myCloseAuctionsActive'],

  searchAuctions: ['searchAuctions'],
})

export const AuctionsTypes = Types
export default Creators

/* ------------- Initial State ------------- */

export const INITIAL_STATE = Immutable({
  data: [],
  listData: [],
  myListAuction: [],
  myListAuctionClose: [],
  myAuctionsHanding: [],
  myAuctionsUnActive: [],
  myCloseAuctionsActive: [],
  searchAuctions: [],
  error: null,
  fetching: false,
  bidSuccess: null,
})

/* ------------- Reducers ------------- */

// we're attempting to login
export const request = (state) => state.merge({ fetching: true })

// we've successfully logged in
export const success = (state, { data }) => state.merge({ fetching: false, error: null, data })

export const bidProductSuccess = (state, { bidSuccess }) => state.merge({ fetching: false, error: null, bidSuccess })

// we've had a problem logging in
export const failure = (state, { error }) => state.merge({ fetching: false, error })

export const bidProductRequest = (state) => state.merge({ })

export const setData = (state, { listData }) => state.merge({ listData })

export const myAuctionSetData = (state, { myListAuction }) => state.merge({ myListAuction })

export const myAuctionCloseSetData = (state, { myListAuctionClose }) => state.merge({ myListAuctionClose })

export const myAuctionHandingSetData = (state, { myAuctionsHanding }) => state.merge({ myAuctionsHanding })

export const myAuctionUnActiveSetData = (state, { myAuctionsUnActive }) => state.merge({ myAuctionsUnActive })

export const myCloseAuctionActiveSetData = (state, { myCloseAuctionsActive }) => state.merge({ myCloseAuctionsActive })

export const searchAuctionsSetData = (state, { searchAuctions }) => state.merge({ searchAuctions })

/* ------------- Hookup Reducers To Types ------------- */

export const reducer = createReducer(INITIAL_STATE, {
  [Types.AUCTIONS_REQUEST]: request,
  [Types.AUCTIONS_SUCCESS]: success,
  [Types.AUCTIONS_FAILURE]: failure,

  [Types.BID_PRODUCT_REQUEST]: request,
  [Types.BID_PRODUCT_SUCCESS]: bidProductSuccess,
  [Types.BID_PRODUCT_FAILURE]: failure,

  [Types.SET_DATA]: setData,

  [Types.MY_AUCTIONS]: myAuctionSetData,

  [Types.MY_AUCTIONS_CLOSE]: myAuctionCloseSetData,

  [Types.MY_AUCTIONS_HANDING]: myAuctionHandingSetData,

  [Types.MY_AUCTIONS_UN_ACTIVE]: myAuctionUnActiveSetData,

  [Types.MY_CLOSE_AUCTIONS_ACTIVE]: myCloseAuctionActiveSetData,

  [Types.SEARCH_AUCTIONS]: searchAuctionsSetData,
})

/* ------------- Selectors ------------- */
