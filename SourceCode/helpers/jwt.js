...
sign = (obj) => {
    let token = jwt.sign(obj, KEY, { expiresIn: expirationTime });
            ...
            .then((result) => {
                if (result.rowCount > 0) {
                    jwt.verify(token, KEY, { ignoreExpiration }, function (error, decoded) {
                        ...
                        return resolve({object: decoded, sessionId: result.rows[0].sessionId});
                    });
                } else {
                    reject({
                        status: 400,
...
module.exports = { sign, verify, refreshToken, removeToken };